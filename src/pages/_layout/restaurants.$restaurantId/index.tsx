...
import { Avatar, Card, Col, Flex, Image, Row, Select, Space, Statistic, Table, Tag, theme, Typography } from "antd"
...
function RouteComponent() {
  ...
  const weeklyChartSeries = useMemo(() => [
    ...
    },
    {
      name: "Доход",
      type: "line" as const,
      data: weeklyData.revenue.map((r) => r / 1000), // Конвертируем в тысячи для читаемости
    },
  ], [])
  ...
  const monthlyChartSeries = useMemo(() => [
    ...
    },
    {
      name: "Доход",
      type: "line" as const,
      data: monthlyData.revenue.map((r) => r / 10000), // Конвертируем в десятки тысяч
    },
  ], [])
  ...
  return (
    <>
            ...
            <Space orientation={"vertical"} style={{ width: "100%" }} size={"middle"}>
              {topTables.map((table) => (
                ...
                >
                  ...
                  <Tag color={table.color}>{table.covers} {"накрытий"}</Tag>
                </Space>
              ))}
            </Space>
          </Card>
      ...
      <Card title={"Карта ресторана"} extra={<Select placeholder={"Выберите этаж"} style={{ width: 150 }}>
        ...
        <Flex justify={"center"} style={{ position: "relative", width: "100%", height: "auto", overflow: "auto" }}>
          <Map />
        </Flex>
      </Card>
    </>
